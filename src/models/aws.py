...
class BDAClient(BaseModel):
    ...
    def from_config(cls, config_file: str) -> "BDAClient":
        ...
        if not found_blueprint:
            print(f"Blueprint not found in project blueprints, trying direct access...")
            ...
            try:
                ...
                if response and 'blueprint' in response:
                    ...
                    print(f"Successfully accessed blueprint directly: {found_blueprint.blueprintName}")
            except Exception as e:
                print(f"Failed to access blueprint directly: {str(e)}")
    ...
    def find_blueprint_by_id(blueprints: List[Blueprint], blueprint_id: str) -> Optional[Blueprint]:
        ...
        for blueprint in blueprints:
            ...
            if blueprint_id.startswith('arn:aws:bedrock'):
                if blueprint_id == arn:
                    return blueprint
            else:
    ...
    def _clean_schema_for_blueprint(self, schema_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "definitions" in cleaned_schema and "properties" in cleaned_schema:
            ...
            flattened_props = [p for p in properties.keys() if "." in p]
            if flattened_props:
    ...
    def delete_test_blueprint(self):
        try:
            ...
            response = self.bda_client.delete_blueprint(
                blueprintArn=self.test_blueprint_arn)
